...
export default function Home() {
  useEffect(() => {
    ...
    window.onscroll = () => {
      ...
      if (window.scrollY >= branches.offsetTop - 600) {
        branches.classList.remove("opacity-0")
        branches.classList.remove("translate-y-10")
        for (let index = 0; index < branches.children[1].children.length; index++) {
          branches.children[1].children[index].classList.remove("opacity-0")
          branches.children[1].children[index].classList.remove("translate-y-10")
        }
      }
    }
